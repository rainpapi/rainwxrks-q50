...
# FIXED: robust file discovery instead of src/com/appgarage/dash/*.java glob
if [ ! -d "src/com/appgarage/dash" ]; then
  echo "ERROR: src/com/appgarage/dash not found"
  echo "--- repo layout ---"
  find src -type f 2>/dev/null | head -n 100 || ls -R
  exit 1
fi

JAVA_FILES=$(find src/com/appgarage/dash -name "*.java" -type f)
if [ -z "$JAVA_FILES" ]; then
  echo "ERROR: no .java files found in src/com/appgarage/dash"
