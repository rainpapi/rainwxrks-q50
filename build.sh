...
echo "== [1/6] generate Android R.java =="
# V3 uses bundled drawable resources through com.rainwxrks.q50.R.
# The lightweight build does not use Gradle, so javac will not get an R class
# unless aapt generates it before Java compilation.
"$AAPT" package -f -m -J build/generated -M AndroidManifest.xml -S res -I "$ANDJAR"
if [ ! -f build/generated/com/rainwxrks/q50/R.java ]; then
  echo "ERROR: aapt did not generate build/generated/com/rainwxrks/q50/R.java"
  find build/generated -type f -maxdepth 6 2>/dev/null || true
  exit 1
fi

echo "== [2/6] javac (release 8) [${APP_NAME}] =="
# Compile the canonical Q50 package directory plus the generated Android R class.
...
echo "  found $(echo "$JAVA_FILES" | wc -l) source files + generated R.java"
# shellcheck disable=SC2086
"$JAVAC" --release 8 -g -d build/classes -classpath "$ANDJAR" build/generated/com/rainwxrks/q50/R.java $JAVA_FILES
echo "  compiled: $(find build/classes -name '*.class' | wc -l) classes"

echo "== [3/6] d8 -> classes.dex (min-api 10) =="
# shellcheck disable=SC2046
"$D8" --min-api 10 --lib "$ANDJAR" --output build/dex $(find build/classes -name '*.class')
ls -l build/dex/classes.dex

echo "== [4/6] package APK (versionCode=$VC) =="
...
echo "== [7/7] wrap into .epk (uses the public OBU cert in keys/obu_cert.pem) =="
PY="python"
