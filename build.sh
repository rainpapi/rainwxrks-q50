...
echo "== [1/6] generate resources + javac (release 8) [${APP_NAME}] =="
# Generate the Android resource symbol class before compiling Java.
# V3 screens reference com.rainwxrks.q50.R.drawable.*, so javac must see R.java first.
...
if [ ! -f "build/generated/com/rainwxrks/q50/R.java" ]; then
  echo "ERROR: aapt did not generate com/rainwxrks/q50/R.java"
  find build/generated -type f -maxdepth 8 2>/dev/null || true
  exit 1
fi
...
R_FILE="build/generated/com/rainwxrks/q50/R.java"
# shellcheck disable=SC2086
"$JAVAC" --release 8 -g -d build/classes -classpath "$ANDJAR" $JAVA_FILES "$R_FILE"
